...
function upgradeHelmDeployment {
    ...
    $helmArguments = "image.repository=index.docker.io/dannybatchrun/${imageName}\,image.tag=${imageTag}\,image.pullPolicy=Always\,service.port=${servicePort}\,livenessProbe.httpGet.path=/health\,livenessProbe.httpGet.port=${servicePort}\,service.type=NodePort"
    helm package .
    kubectl scale --replicas=0 "deployment/${imageName}" -n "${imageName}"
    helm upgrade "${imageName}" . --set $($helmArguments -split ",") -n "${imageName}"
